...
var (
	trueConditions = []corev1.NodeConditionType{
		corev1.NodeReady,
	}
	falseConditions = []corev1.NodeConditionType{
		diskFullCondition,
	}
)
...
func isNodeUnhealthy(n corev1.Node) bool {
	...
	for _, trueCondition := range trueConditions {
		for _, c := range n.Status.Conditions {
			if c.Type == trueCondition && c.Status != corev1.ConditionTrue {
	...
	for _, falseCondition := range falseConditions {
		for _, c := range n.Status.Conditions {
			if c.Type == falseCondition && c.Status == corev1.ConditionTrue {
