...
func (d *Detector) DetectBadNodes(ctx context.Context) ([]corev1.Node, error) {
	d.logger.Debugf(ctx, "Detecting unhealthy nodes using trueConditions = %v and falseConditions = %v", d.trueConditions, d.falseConditions)
	...
	for i, n := range nodeList.Items {
		notReadyTickCount, updated := d.nodeNotReadyTickCount(ctx, n)
...
func (d *Detector) isNodeUnhealthy(ctx context.Context, n corev1.Node) bool {
	// trueConditions have to be true, otherwise node has to be considered unhealthy.
	for _, trueCondition := range d.trueConditions {
		for _, c := range n.Status.Conditions {
			if c.Type == trueCondition && c.Status != corev1.ConditionTrue {
				...
				if time.Since(c.LastHeartbeatTime.Time) >= nodeNotReadyDuration {
					d.logger.Debugf(ctx, "node %s is unhealthy because we expected condition %s to be true, but was false")
					return true
				}
			}
		}
	...
	for _, falseCondition := range d.falseConditions {
		for _, c := range n.Status.Conditions {
			if c.Type == falseCondition && c.Status == corev1.ConditionTrue {
				// we want condition to be false, but it's not.
				if time.Since(c.LastHeartbeatTime.Time) >= nodeNotReadyDuration {
					d.logger.Debugf(ctx, "node %s is unhealthy because we expected condition %s to be false, but was true")
					return true
				}
			}
		}
...
func (d *Detector) nodeNotReadyTickCount(ctx context.Context, n corev1.Node) (int, bool) {
	var err error
	updated := false
